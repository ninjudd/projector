...
function renderWalkthrough(data: WalkthroughData): void {
  ...
  function num(n: number): string { return Number(n).toLocaleString('en-US'); }
  function short(sha: string | undefined): string { return String(sha || '').slice(0, 9); }
...
function wireWalkthrough(): void {
  ...
  function get(k: string): string | null { try { return localStorage.getItem(KEY + k); } catch (e) { return null; } }
  function set(k: string, v: boolean): void { try { if (v) localStorage.setItem(KEY + k, '1'); else localStorage.removeItem(KEY + k); } catch (e) {} }
  ...
  document.querySelectorAll<HTMLElement>('.copypath').forEach(function (b) {
    b.addEventListener('click', function (ev) {
      ...
      function fallback(): void {
        const ta = document.createElement('textarea'); ta.value = path; ta.setAttribute('readonly', ''); ta.style.position = 'fixed'; ta.style.opacity = '0';
  ...
  function splitLines(s: string): string[] {
    ...
    while ((m = re.exec(s)) !== null) {
      const t = m[0];
      if (t === '\n') {
        for (let i = 0; i < open.length; i++) cur += '</span>';
        out.push(cur);
        cur = open.join('');
      } else if (t === '</span>') {
        open.pop(); cur += t;
    ...
    for (let j = 0; j < open.length; j++) cur += '</span>';
    out.push(cur);
    return out;
  }
  function hl(text: string, lang: string): string[] | null {
    try { return splitLines(window.hljs!.highlight(text, { language: lang, ignoreIllegals: true }).value); }
    catch (e) { return null; }
  }
  function highlightFile(f: HTMLElement): void {
    const hljs = window.hljs;
    if (f.dataset.hl || !hljs) return;
    ...
    f.querySelectorAll('table.diff').forEach(function (tbl) {
      ...
      rows.forEach(function (r) {
        const src = r.querySelector('.src'); const text = src ? src.textContent || '' : '';
...
if (node) {
  ...
  fetch((mount && mount.dataset.src) || 'data.json')
    ...
    .then(function (data: WalkthroughData) {
