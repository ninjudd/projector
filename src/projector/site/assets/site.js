...
(function () {
    ...
    function ownerOf(path) {
        let best = null;
        for (const p of site.projects) {
            if (path.startsWith(folderOf(p)) && (best === null || p.name.length > best.name.length))
                best = p;
        }
        return best;
    }
    ...
    function nav(active) {
        ...
        if (site.readme !== null || site.docs.length > 0)
    ...
    function tree(items, prefix, top, current) {
        ...
        items.forEach(function (item) {
            ...
            if (node !== rootNode && /^readme\.md$/i.test(name))
                node.index = item;
            else
    ...
    function showPage(active, path, title, before, side) {
        if (isHtml(path))
            showHtml(active, path, title, before, side);
        else
            showDocument(active, path, title, before, side);
    }
    ...
    function showProjects() {
        ...
        if (filter === null)
            throw new Error('The projects view lost the filter it just drew');
    ...
    function showProjectFile(path) {
        ...
        site.projects.forEach(function (p) {
            if (p === top || !p.path.startsWith(prefix))
                return;
    ...
    function showSearch(query) {
        ...
        if (input === null || list === null)
            throw new Error('The search view lost the elements it just drew');
        input.value = query;
        function render() {
            if (input === null || list === null)
                return;
    ...
    function route() {
        ...
        switch (rel) {
            ...
            case 'docs':
                ...
                return;
            case 'search':
                showSearch(new URLSearchParams(location.search).get('q') ?? '');
                return;
        }
    ...
    document.addEventListener('click', function (event) {
        ...
        if (root === null || event.defaultPrevented || event.button !== 0 || event.metaKey || event.ctrlKey || event.shiftKey || event.altKey)
            return;
    ...
    document.addEventListener('submit', function (event) {
        ...
        if (root === null || !(form instanceof HTMLFormElement) || !form.classList.contains('sitesearch'))
            return;
        event.preventDefault();
        const input = form.querySelector('input');
        if (input === null)
            throw new Error('The search form has no input');
        const q = input.value.trim();
    ...
    fetch(`${base}site.json`)
        ...
        if (site.readme !== null)
            pageFiles.add(site.readme);
        if (site.projectsReadme !== null)
            pageFiles.add(site.projectsReadme);
        ...
        window.addEventListener('resize', function () { if (fitFrame !== null)
            fitFrame(); });
        route();
    }, function (error) {
