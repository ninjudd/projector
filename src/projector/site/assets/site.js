...
(function () {
    ...
    function ownerOf(path) {
        let best = null;
        for (const p of site.projects) {
            if (path.indexOf(folderOf(p)) === 0 && (!best || p.name.length > best.name.length))
                best = p;
        }
        return best;
    }
    ...
    function renderMarkdown(text, path) {
        ...
        box.querySelectorAll('a[href]').forEach(function (a) {
            ...
            if (pageFiles[target])
                a.setAttribute('href', routeFor(target) + hash);
            else if (siteFiles[target])
                a.setAttribute('href', contentUrl(target));
            else
                a.setAttribute('href', repoUrl('blob', target));
        });
        box.querySelectorAll('img[src]').forEach(function (img) {
            const src = img.getAttribute('src') || '';
            if (/^[a-z][a-z0-9+.-]*:/i.test(src) || src.charAt(0) === '/')
                return;
        ...
        if (hljs)
            box.querySelectorAll('pre code').forEach(function (code) { hljs.highlightElement(code); });
        return box;
    }
    ...
    function nav(active) {
        ...
        if (site.projects.length)
            items.push([base, 'Projects', 'projects']);
        if (site.reviews.length)
            items.push([base + 'reviews/', 'Reviews', 'reviews']);
        if (site.readme || site.docs.length)
            items.push([base + 'docs/', 'Docs', 'docs']);
    ...
    function tree(items, prefix, top, current) {
        const rootNode = { dirs: {}, files: [], index: null };
        items.forEach(function (item) {
            ...
            if (node !== rootNode && /^readme\.md$/i.test(parts[parts.length - 1]))
                node.index = item;
            else
    ...
    function showDocument(active, path, title, before, side) {
        ...
        loadMarkdown(path).then(function (text) {
            ...
            if (before)
                main.insertBefore(before, main.firstChild);
    ...
    function showProjects() {
        ...
        const main = frame('projects', 'Projects', '<h1>Projects</h1><p class="note">' + site.projects.length + ' projects under <span class="mono">' + esc(site.projectsDir) + '</span>. ' +
        ...
        main.querySelector('.filter').addEventListener('input', function (event) {
            ...
            main.querySelectorAll('tr[data-search]').forEach(function (row) {
    ...
    function showProjectFile(path) {
        ...
        site.projects.forEach(function (p) {
            if (p === top || p.path.indexOf(prefix) !== 0)
                return;
            items.push({ path: p.path, title: p.title });
        });
        site.projects.forEach(function (p) {
            if (p.path.indexOf(prefix) === 0)
                p.files.forEach(function (f) { items.push(f); });
        });
    ...
    function snippet(text, words) {
        ...
        if (at < 0)
            at = words.reduce(function (first, w) { const i = lower.indexOf(w); return i >= 0 && (first < 0 || i < first) ? i : first; }, -1);
    ...
    function showSearch(query) {
        const main = frame('search', query ? 'Search: ' + query : 'Search', '<h1>Search</h1><input class="filter" type="search" aria-label="Search the site" placeholder="Search the projects and docs">' +
            '<ol class="results"></ol>');
        const input = main.querySelector('.filter');
        const list = main.querySelector('.results');
        input.value = query || '';
        function render() {
    ...
    function route() {
        ...
        if (!rel)
            return site.projects.length ? showProjects() : showDocs(site.readme);
        if (rel === 'projects')
            return showProjects();
        if (rel === 'reviews')
            return showReviews();
        if (rel === 'docs')
            return showDocs(site.readme);
        if (rel === 'search')
            return showSearch(new URLSearchParams(location.search).get('q') || '');
        const file = routes[rel + '/'];
        if (file)
            return inProjects(file) ? showProjectFile(file) : showDocs(file);
    ...
    document.addEventListener('click', function (event) {
        ...
        if (!root || event.defaultPrevented || event.button !== 0 || event.metaKey || event.ctrlKey || event.shiftKey || event.altKey)
            return;
    ...
    document.addEventListener('submit', function (event) {
        ...
        if (!root || !form.classList || !form.classList.contains('sitesearch'))
            return;
        event.preventDefault();
    ...
    fetch(base + 'site.json')
        ...
        if (site.readme)
            pageFiles[site.readme] = true;
        if (site.projectsReadme)
            pageFiles[site.projectsReadme] = true;
        ...
        window.addEventListener('resize', function () { if (fitFrame)
            fitFrame(); });
        route();
    }, function (error) {
