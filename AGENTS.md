...
repository's deploy and every CLI install reads them from a Git tree. The
npm project that builds them is `src/projector/site/`, so run npm there.
...
skips that check where `npm ci` has not run in `src/projector/site/`. Resolve a conflict in a compiled
